...
//        sortList.fastSort(a,0,a.length-1);
        sortList.shellSort(a);
...
//    }
    ...
    public void fastSort(int[] array, int low, int hi) {
        if (low > hi) {
            return;
        }
        int i = low, j = hi, temp = array[low];
        while (i != j) {
    ...
    public void heapSort(int[] array) {
        int i = array.length / 2 + 1;
        for (; i >= 0; i--) {
            adjustSort(array, i, array.length);
        }
        for (int j = array.length - 1; j >= 0; j--) {
            swap(array, 0, j);
